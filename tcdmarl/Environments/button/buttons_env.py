...
from tcdmarl.Environments.common import STR_TO_ACTION, DecentralizedEnv,ButtonsMap
...
class ButtonsEnv(DecentralizedEnv):  # TODO rename to DecentralizedButtonsEnv
    """
    ...
    def use_prm(self, value: bool) -> "ButtonsEnv":
        if not value:
            return self
    ...
    def environment_step(self, s: int, a: int) -> Tuple[int, List[str], int]:
        ...
        s_next : int
            Index of next state.
        """
        buttons_state = self.map.compute_state(self.agent_id, self.get_old_u(self.u))
        s_next, last_action = self.map.get_next_state(
            s, a, self.agent_id, buttons_state
        )
    ...
    def get_mdp_label(self, _s: int, s_next: int, u: int) -> List[str]:
        """
        ...
        if self.agent_id == 0:
            if u == 0:
                if (row, col) == self.map.env_settings["yellow_button"]:
                    # print('l in line 152_buttons_env.py: ', "hi_temp")
                    l.append("by")
            ...
            if u == 1 and (row, col) == self.map.env_settings["green_button1"]:
                # print("l in line 162_buttons_env.py: ", "hi2")
                l.append("bg1")
            if u == 1 and (row, col) == self.map.env_settings["green_button2"]:
                l.append("bg2")
            if u == 2 and (row, col) == self.map.env_settings["red_button"]:
                l.append("a2br")
            if u == 3:
            ...
            if u == 0:
                ...
                if np.random.random() <= SYNCHRONIZATION_THRESH:
                    l.append("bg2")
            if u == 1 and (row, col) == self.map.env_settings["red_button"]:
                l.append("a3br")
            if u == 2:
                if not((row, col) == self.map.env_settings["red_button"]):
                    l.append("a3lr")
...
def play():
    ...
    game = ButtonsEnv(tester.rm_learning_file_list[agent_id], agent_id, env_settings)
