...
from tcdmarl.tcrl.custom_dfas_neurips import dfa_paper_no_br_after_f
...
def buttons_config(num_times: int, use_tlcd: bool, step_unit_factor: int) -> Tester:
    ...
    env_settings["F1"] = (6, 5)
    env_settings["F2"] = (6, 7)
    env_settings["enable_f2"] = True

    env_settings["p"] = 0.98

    # In the buttons env, F's do not represent sinks, but signal the agent entering a fruitless region
    env_settings["sinks"] = []

    if use_tlcd:
        tlcd = dfa_paper_no_br_after_f()
    else:
        tlcd = None
