...
class RMEnvWrapper(gym.Wrapper):
    def __init__(self, env, reward_machine: RewardMachine):
        ...
        self.runner: RewardMachineRunner = RewardMachineRunner(self.reward_machine)
        self.label_history: list[frozenset[str]] = []
        self.num_rm_states = len(self.reward_machine.nonterminal_states)
        ...
        for u_id in self.reward_machine.nonterminal_states:
    ...
    def step(self, action):
        ...
        true_rew = self.runner.reward_machine.true_reward
        info["true_reward"] = true_rew

        done = rm_done or env_done
        rm_obs = self.get_observation(next_obs, self.runner.current_state, done)
    ...
    def _get_crm_experience(
        self, obs, action, next_obs, env_done: bool, true_props: frozenset[str]
    ):
