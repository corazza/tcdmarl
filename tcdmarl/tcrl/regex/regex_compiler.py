...
class DFANode:
    ...
    def t(self, input_symbol: frozenset[str], node: "DFANode"):
        ...
        assert (
            input_symbol not in self.transitions
        ), f"failed: {input_symbol} not in {self.transitions}"
...
def epsilon_once(from_states: frozenset[NFANode]) -> frozenset[NFANode]:
    next_set = frozenset([s.along_symbol(frozenset({"*"})) for s in from_states])
    new_states = frozenset(itertools.chain.from_iterable(next_set))
...
def along_symbol(
    from_states: frozenset[NFANode], input_symbol: frozenset[str]
) -> frozenset[NFANode]:
...
def next_superstate(
    states: frozenset[NFANode], input_symbol: frozenset[str]
) -> frozenset[NFANode]:
...
def to_dfa(compiled_nfa: CompileStateNFA, node_creator: NodeCreator) -> CompileStateDFA:
    first_epsilon: frozenset[NFANode] = epsilon(frozenset({compiled_nfa.initial}))
    ...
    state_dict: dict[frozenset[NFANode], DFANode] = {first_epsilon: initial_dfa}
...
def to_nfa(dfa: CompileStateDFA, node_creator: NodeCreator):
    ...
    while len(to_visit_dfa) > 0:
        ...
        if visiting_dfa.id not in dfa_id_to_nfa_node:
            nfa_visiting: NFANode = node_creator.convert_node_to_nfa(visiting_dfa)
...
def nfa_union(
    nfas: list[CompileStateNFA], node_creator: NodeCreator
) -> CompileStateNFA:
