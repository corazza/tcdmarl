...
def run_ihrl_training(epsilon, tester, agent_list, show_print=True):
    ...
    if tester.experiment == "rendezvous":
        training_env = MultiAgentGridWorldEnv(
    ...
    for t in range(num_steps):
        ...
        for i in range(num_agents):
            if t == 0:
                ...
                avail_meta_action_indeces = training_env.get_avail_meta_action_indeces(
                    i
                )
                meta_action = agent_list[i].get_next_meta_action(
                    current_meta_state,
            ...
            if agent_list[i].option_complete:
                ...
                current_meta_state = training_env.get_meta_state(i)
                meta_action = agent_list[i].options_list.index(
                    agent_list[i].current_option
                )
                agent_list[i].update_meta_q_function(
                    option_start_state,
                    current_meta_state,
                    meta_action,
                    mc_rewards[i],
        ...
        if (
            ...
            for i in range(num_agents):
                ...
                agent_copy = IhrlAgent(
                    options_list, s_i, num_states, num_meta_states, actions, agent_id
                )
            ...
            testing_reward, trajectory, testing_steps = run_ihrl_test(
                agent_list_copy,
                tester,
                learning_params,
                testing_params,
...
def run_ihrl_test(agent_list, tester, learning_params, testing_params, show_print=True):
    """
    ...
    for t in range(testing_params.num_steps):
        ...
        for i in range(num_agents):
            if t == 0:
                ...
                meta_action = agent_list[i].get_next_meta_action(
                    current_meta_state, avail_meta_action_indeces, -1, learning_params
                )
                agent_list[i].current_option = agent_list[i].options_list[meta_action]
...
def run_ihrl_experiment(tester, num_agents, num_times, show_print=True):
    """
    ...
    for t in range(num_times):
        ...
        for i in range(num_agents):
            ...
            agent_list.append(
                IhrlAgent(options_list, s_i, num_states, num_meta_states, actions, i)
            )
        ...
        while not tester.stop_learning():
            ...
            run_ihrl_training(epsilon, tester, agent_list, show_print=show_print)
